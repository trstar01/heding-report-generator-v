...
export const config = {
  api: { bodyParser: { sizeLimit: '20mb' } } // 포트폴리오 원본 파일(base64, 최대 10MB)까지 수용
};
...
export default async function handler(req, res) {
  ...
  try {
    const { resumeText, portfolioText, portfolioFileBase64, portfolioFileMediaType, inputs } = req.body;
...
2. resumeEdits의 original은 반드시 이력서 원문에서 실제로 발췌한 내용이어야 한다. portfolioImprovements의 original은, 포트폴리오가 텍스트 기반이면 원문을 실제로 발췌하고, 이미지 기반(첨부 파일 직접 분석)이면 실제로 확인한 페이지·프로젝트 내용을 구체적으로 서술한다. 어느 경우든 지어내지 않는다 — 확신이 없으면 해당 항목을 생략한다.
...
8. inputs 필드는 만들지 않는다 (서버에서 별도 처리).`;
    ...
    // 포트폴리오가 이미지 기반이면, 프롬프트 텍스트와 함께 원본 PDF 파일을 문서 블록으로 첨부
    const requestContent = useVisionForPortfolio
      ? [
          { type: 'document', source: { type: 'base64', media_type: portfolioFileMediaType, data: portfolioFileBase64 } },
